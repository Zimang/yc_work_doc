...
public class PriorityExecutor extends ThreadPoolExecutor  {
     ...
     * 创建线程工厂
     */
    private static final ThreadFactory THREAD_FACTORY = new SvThreadFactory();


    /**
     * 线程队列方式 先进先出
     */
    private static final Comparator<Runnable> FIFO = (lhs, rhs) ->  {
        if (lhs instanceof PriorityRunnable && rhs instanceof PriorityRunnable)  {
            ...
            int result = lpr.getPriority().ordinal() - rpr.getPriority().ordinal();
    ...
    public PriorityExecutor(int poolSize, boolean fifo)  {
        this(poolSize, MAXIMUM_POOL_SIZE, KEEP_ALIVE, TimeUnit.SECONDS, new PriorityBlockingQueue<Runnable>(MAXIMUM_POOL_SIZE, fifo ? FIFO : LIFO), THREAD_FACTORY);
    }
